...
All experiments are conducted on the \textbf{BFT validation set} (25 videos, 5020 frames) following the split provided by NetTrack/BFT.
We process each sequence independently and report the average tracking performance averaged over all sequences.
Our OpenCV pipeline is \textbf{training-free} and runs in an \textbf{online} manner: at frame $t$ it only uses information from frames $\le t$.

\paragraph{Annotation and file format.}
BFT annotations are provided in COCO-style JSON with per-frame bounding boxes and \texttt{traceID}s.
For evaluation, we convert both ground truth and predictions into \textbf{MOTChallenge} text format:
\texttt{frame, id, x, y, w, h} (pixel coordinates).
This unified interface is shared by our OpenCV pipeline and the NetTrack baseline, enabling identical evaluation scripts.

\paragraph{Preprocessing and camera-motion compensation.}
The OpenCV pipeline uses two feature channels:
(i) a smoothed \textbf{intensity} image for motion differencing and optical flow;
(ii) a \textbf{LoG} (Laplacian-of-Gaussian) feature map for robust camera-motion estimation under illumination changes.
We also apply a \textbf{hard valid-mask} for subtitles/invalid regions and a \textbf{soft specular-mask} to suppress reflections/glints.
Specular thresholds are adapted from the first few frames of each video, while subtitle parameters are fixed across all sequences.

\paragraph{Hyperparameters.}
We use a fixed configuration for all sequences (no per-video manual tuning beyond Step-1 adaptive glare statistics).
Key parameters are summarized in Tab.~\ref{tab:opencv_settings}.
To prevent the table from being truncated in a two-column layout, we present it as a \texttt{table*} spanning both columns.
...
High-thresh percentile $=85$; high/low factors $=0.7/0.3$;
area filter: min $25$, max $3000$, single-bird max $800$;
morphology kernels: recon $3$, close $5$, open $3$ \\
\hline
Step-4 refinement &
\texttt{min\_area\_ratio}=5e{-}5; \texttt{max\_box\_area\_ratio}=0.3; \texttt{min\_fg\_pixels}=20;
\texttt{fg\_ratio\_min}=0.2; \texttt{fill\_ratio\_min}=0.10; \texttt{spec\_frac\_max}=0.6;
\texttt{min\_side}=4; aspect ratio in $[0.10,10.0]$; \texttt{CENTER\_FRAC}=0.3 \\
\hline
Step-5 association &
IoU gate $\ge 0.3$; center-dist gate $\le 1.5 \times$ diag;
cost weights: $w_\text{iou}=0.7$, $w_\text{dist}=0.3$;
\texttt{max\_age}=5, \texttt{min\_hits}=2;
birth gate: IoU$<0.10$ and dist$>0.8\times$diag \\
\hline
Step-5 IK (Iterative LK) &
Sparse pyramidal LK within each track ROI;
forward-backward check for outlier rejection;
robust median flow update for box refinement;
re-initialize keypoints when too few remain or after long occlusion \\
\hline
\end{tabular}
\caption{Experimental hyperparameters of the OpenCV-based pipeline (fixed across all sequences).}
\label{tab:opencv_settings}
\end{table*}

\paragraph{Evaluation metrics.}
We report classic multi-object tracking metrics including \textbf{MOTA}, \textbf{IDF1}, \textbf{ID switches}, \textbf{false positives}, and \textbf{misses}.
Associations are computed using an IoU-based matching threshold of 0.5 (i.e., matches require IoU$\ge 0.5$ in the evaluation distance matrix).
For completeness, we also compute \textbf{HOTA} using TrackEval on the same MOT-format files when available, keeping the IoU threshold consistent.
...
Given an input frame $I_t$, we compute two channels:
(i) a smoothed grayscale image $G_t$ used for motion differencing and optical flow;
(ii) a LoG feature map $L_t$ used for robust camera-motion estimation under illumination changes.
We further construct a hard \texttt{valid\_mask} to remove subtitles/invalid pixels and a soft \texttt{spec\_mask} to suppress specular highlights.
Masked regions are filled (blur/inpaint style) to avoid introducing sharp edges that bias motion estimation.

\paragraph{Step-2: camera motion compensation.}
We estimate the dominant inter-frame camera translation $T_{t-1\rightarrow t}$ from $(L_{t-1},L_t)$ using a coarse-to-fine strategy:
phase correlation provides a fast initialization, followed by ROI-consensus refinement with outlier rejection; an ECC-based alignment is used as fallback.
We warp $G_{t-1}$ to $\tilde{G}_{t-1}$ and compute aligned motion cues from $(\tilde{G}_{t-1}, G_t)$.

\paragraph{Step-3: motion-based candidate generation.}
We compute a motion map $D_t = |\tilde{G}_{t-1}-G_t|$.
To retain small birds while suppressing noise, we apply a dual-threshold reconstruction:
high-threshold pixels form seeds and low-threshold pixels provide support regions.
After morphological cleanup, connected components are filtered by area and simple geometry, yielding coarse candidate boxes $\mathcal{B}_t^{raw}$.
...
This produces refined detections $\mathcal{B}_t$.
