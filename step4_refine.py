...
def step4_refine(bgr, mask_fg, boxes_raw, spec_mask, cfg: RefineConfig):
    ...
    for (x, y, w, h) in boxes_raw:
        ...
        _, sure_fg = cv2.threshold(dist, cfg.CENTER_FRAC * dmax, 1, cv2.THRESH_BINARY)
        ...
        if num_markers <= 1:
            markers = np.ones_like(fg, dtype=np.int32)   # background=1
            markers[fg > 0] = 2                          # foreground=2
            num_markers = 2
        else:
