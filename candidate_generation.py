...
# output: motion candidate mask + boxes
...
class CandidateGenConfig:
    ...
    # otherwise it is a soft mask and only suppress spec region)
...
def mad_threshold_u8(img_u8: np.ndarray, mask_u8: Optional[np.ndarray], k: float, tmin: int, tmax: int) -> Tuple[int, Dict[str, Any]]:
...
def apply_spec_weight(diff_u8: np.ndarray, spec_mask: Optional[np.ndarray], weight: float, valid_mask: Optional[np.ndarray]) -> np.ndarray:
    """
...
def box_score_edge_change(curr_log: Optional[np.ndarray], prev_log_aligned: Optional[np.ndarray], box: Tuple[int, int, int, int]) -> float:
...
def generate_motion_candidates(
    ...
    spec_ratio = ratio_in_valid((spec_mask == 0) if spec_mask is not None else np.zeros((H, W), bool), valid_mask)
    ...
    fused = binary_cleanup(fused, k_open, k_close)
    ...
    if cfg.use_bridge:
        ...
        fused = bridge(fused, k_bd, k_bc)

    if cfg.dilate_ksize:
        fused = maybe_dilate(fused, scaled_k(cfg.dilate_ksize, H, W, cfg.kernel_scale_ref))
    ...
    update_persistence(gen, fused, valid_mask)
    ...
    debug["cc"] = {
        ...
        "final_fg_ratio_valid": float(ratio_in_valid(fused > 0, valid_mask)),
    }
